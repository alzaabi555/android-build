...
const REWARDS: RewardItem[] = [
    { id: '1', title: 'تغيير المكان', cost: 15, icon: '🪑', bg: 'bg-blue-100' },
    { id: '2', title: 'قائد الطابور', cost: 10, icon: '🚩', bg: 'bg-emerald-100' },
    { id: '3', title: 'إعفاء واجب', cost: 50, icon: '📝', bg: 'bg-purple-100' },
    { id: '4', title: 'مساعد المعلم', cost: 25, icon: '👨‍🏫', bg: 'bg-amber-100' },
    { id: '5', title: 'قلم مميز', cost: 30, icon: '✏️', bg: 'bg-rose-100' },
    { id: '6', title: 'نجمة الفصل', cost: 40, icon: '⭐', bg: 'bg-yellow-100' },
];

const LEVELS = [
    { name: 'مبتدئ', min: 0, color: 'text-gray-500', bg: 'bg-gray-100' },
    { name: 'مغامر', min: 10, color: 'text-blue-600', bg: 'bg-blue-100' },
    { name: 'فارس', min: 30, color: 'text-indigo-600', bg: 'bg-indigo-100' },
    { name: 'بطل', min: 60, color: 'text-purple-600', bg: 'bg-purple-100' },
    { name: 'أسطورة', min: 100, color: 'text-amber-600', bg: 'bg-amber-100' },
];
...
const GamificationHub: React.FC<GamificationHubProps> = ({ students, classes, onUpdateStudent }) => {
  ...
  const getAvatarUrl = (id: string) => {
      // استخدام خدمة DiceBear لتوليد شخصيات بنمط "المغامر" (Adventurer)
  ...
  // ترتيب الطلاب حسب الرصيد الحالي (الأغنى أولاً)
  ...
  return (
    <div className="space-y-4 pb-24 md:pb-8 animate-in fade-in duration-500">
      ...
      <div className="flex gap-3 overflow-x-auto pb-2 custom-scrollbar">
         <button 
            ...
            className={`px-4 py-2 rounded-xl text-xs font-black whitespace-nowrap transition-all ${selectedClass === 'all' ? 'bg-gray-800 text-white shadow-md' : 'bg-white text-gray-500 border border-gray-100'}`}
         >
            الكل
         </button>
         {classes.map(c => (
            <button 
                ...
                className={`px-4 py-2 rounded-xl text-xs font-black whitespace-nowrap transition-all ${selectedClass === c ? 'bg-indigo-600 text-white shadow-md' : 'bg-white text-gray-500 border border-gray-100'}`}
            >
                {c}
            </button>
         ))}
          ...
          {sortedStudents.map((student, idx) => {
              ...
              return (
                  <div 
                    ...
                    className="bg-white rounded-3xl p-4 border border-gray-100 shadow-sm relative overflow-hidden group cursor-pointer active:scale-95 transition-all"
                  >
                      {/* Rank Badge */}
                      <div className="absolute top-3 left-3 w-6 h-6 bg-gray-100 rounded-full flex items-center justify-center text-[10px] font-black text-gray-500 z-10">
                          #{idx + 1}
                      </div>

                      <div className="flex flex-col items-center">
                          <div className="w-20 h-20 rounded-full bg-indigo-50 border-4 border-white shadow-md mb-3 overflow-hidden">
                              <img src={getAvatarUrl(student.id)} alt="avatar" className="w-full h-full object-cover" />
                          </div>
                          <h3 className="font-black text-gray-800 text-sm text-center mb-1 truncate w-full">{student.name}</h3>
                          
                          <div className={`px-2 py-0.5 rounded-md text-[9px] font-black mb-3 ${level.bg} ${level.color}`}>
                              {level.name}
                          </div>

                          <div className="w-full bg-gray-50 rounded-xl p-2 flex items-center justify-between">
                              <span className="text-[9px] font-bold text-gray-400">الرصيد</span>
